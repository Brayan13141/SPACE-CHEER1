...
class OrderMeasurementService:
    ...
    def populate_from_profile(order_item, athlete):
        ...
        if order.order_type == "TEAM":
            if athlete not in order.owner_team.members.all():
                raise ValidationError("Atleta inválido para esta orden")
    ...
    def save_measurement(athlete_item, field, value):
        ...
        if not order.can_edit_measurements():
            raise ValidationError("Las medidas no están abiertas para edición")
